...
const CircularCTA = () => (
  <div className="relative w-40 h-40 flex items-center justify-center">
    ...
    >
      <svg viewBox="0 0 100 100" className="w-full h-full text-orange-600/60 font-medium tracking-widest text-[8px]">
        <path
          id="circlePath"
    ...
    <div className="w-20 h-20 rounded-full bg-orange-600 flex items-center justify-center text-white font-bold text-center leading-none z-10 shadow-[0_0_30px_rgba(234,88,12,0.5)]">
      Start<br />Project
    </div>
  </div>
);
...
const PointCallout = ({ title, description, side = "left" }: { title: string, description: string, side?: "left" | "right" }) => (
  <motion.div
      ...
      {side === "right" && (
        <div className="mt-1 relative">
          ...
          <svg className="absolute top-1.5 left-[-60px] w-[60px] h-[40px] overflow-visible">
            <line x1="60" y1="0" x2="0" y2="40" stroke="white" strokeWidth="1" strokeOpacity="0.3" />
          </svg>
        </div>
      )}
      <div className="space-y-2">
        <h4 className="text-xl font-bold text-white tracking-tight">{title}</h4>
        <p className="text-zinc-500 text-[10px] leading-relaxed font-medium">{description}</p>
      </div>
      {side === "left" && (
        <div className="mt-1 relative">
          <div className="w-3 h-3 rounded-full bg-white shadow-[0_0_10px_white]" />
          <svg className="absolute top-1.5 left-3 w-[60px] h-[40px] overflow-visible">
            <line x1="0" y1="0" x2="60" y2="-40" stroke="white" strokeWidth="1" strokeOpacity="0.3" />
          </svg>
        </div>
      )}
    </div>
...
export const ServicesSection = () => {
  return (
    <Section className="bg-black flex flex-col justify-center min-h-screen py-24 px-6 md:px-12 overflow-hidden">
      {/* Background Lighting */}
      ...
      <div className="relative z-10 max-w-7xl mx-auto w-full h-full flex flex-col justify-between min-h-[80vh]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-start">
          <div className="space-y-6">
            <h2 className="text-6xl md:text-9xl font-black font-display tracking-tighter uppercase leading-[0.8] text-zinc-100">
              Precision <br />
              <span className="text-zinc-400">In Time</span>
            </h2>
            <p className="text-zinc-500 text-sm max-w-xs leading-relaxed font-medium italic">
              / From classic elegance to modern innovation, our services are crafted to elevate your brand presence. /
            </p>
          </div>
        ...
        <div className="flex flex-col md:flex-row items-end justify-between gap-12 pt-12 border-t border-zinc-900">
          <div className="flex gap-16">
            <div className="space-y-1">
              <div className="text-4xl font-bold tracking-tighter text-white">4.5k+</div>
              <div className="text-[10px] font-medium tracking-widest text-zinc-500 uppercase">Satisfied Clients</div>
            </div>
            <div className="h-10 w-px bg-zinc-800 self-center" />
            <div className="space-y-1">
              <div className="text-4xl font-bold tracking-tighter text-white">10 year</div>
              <div className="text-[10px] font-medium tracking-widest text-zinc-500 uppercase">Market Expertise</div>
            </div>
          </div>

          <div className="flex items-center gap-3 text-red-500">
            <Star size={12} fill="currentColor" />
            <span className="text-[8px] font-bold tracking-[0.5em] uppercase text-zinc-600">Established 2014</span>
          </div>
        </div>
